...
docker run --rm -v src:/src -it ubuntu bash # run interact shell, remove the container after it stops
...
docker rm $(docker ps -aq)                  # Remove all containers from this machine
...
docker logs <hash>                          # see docker logs
...
curl -sSL https://get.docker.com/ | sh                   # install docker for linux
...
RUN {apt commands} \
  && apt-get clean \
  && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* # Cleaning APT in a RUN layer
